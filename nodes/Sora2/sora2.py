...
from .kuai_utils import (env_or, ensure_list_from_urls,
                         http_headers_auth_only, http_headers_json, json_get,
                         SORA2_MODELS, get_duration_for_sora2_model,
                         SORA2_DURATION_OPTIONS, validate_sora2_duration,
...
class SoraCreateVideo:
    ...
    def INPUT_TYPES(cls):
        return {
            "required": {
                ...
                "duration_sora2": (SORA2_DURATION_OPTIONS, {"default": "15", "tooltip": "sora-2时长(秒)"}),
                "duration_sora2pro": (SORA2_DURATION_OPTIONS, {"default": "15", "tooltip": "sora-2-pro时长(秒)"}),
            ...
            "optional": {
                "duration": (SORA2_DURATION_OPTIONS, {"default": "", "tooltip": "统一视频时长(秒)，填写后覆盖上方模型时长"}),
                "custom_model": ("STRING", {
                    "default": "",
    ...
    def create(self, images, prompt="", model="sora-2-all", duration_sora2="15", duration_sora2pro="15",
        ...
        selected_duration = (
            ...
            else get_duration_for_sora2_model(actual_model, duration_sora2, duration_sora2pro)
        )
        ...
        try:
            resp = requests.post(endpoint, headers=http_headers_json(api_key), json=payload, timeout=int(timeout))
...
class SoraQueryTask:
    ...
    def query(self, task_id, api_base="https://ai.kegeai.top", api_key="", wait=True, poll_interval_sec=5, timeout_sec=600):
        api_key = env_or(api_key, "KUAI_API_KEY")
...
class SoraCreateAndWait:
    ...
    def run(self, images, prompt, model="sora-2-all", duration_sora2="15", duration_sora2pro="15",
            custom_model="",
            api_base="https://ai.kegeai.top", api_key="", orientation="portrait", size="large", watermark=False,
...
class SoraText2Video:
    ...
    def create(self, prompt, model="sora-2-all", duration_sora2="15", duration_sora2pro="15", seed=0,
        ...
        # 使用中心化函数获取时长
        duration = validate_sora2_duration(
            actual_model,
            get_duration_for_sora2_model(actual_model, duration_sora2, duration_sora2pro),
        )
...
NODE_DISPLAY_NAME_MAPPINGS = {
    "SoraCreateVideo": "sora图文视频生成-统一视频格式",
